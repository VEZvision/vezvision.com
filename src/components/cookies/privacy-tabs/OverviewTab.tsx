...
export function OverviewTab({
  ...
  return (
    <div id="panel-overview" role="tabpanel" className="p-6 space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('privacy.overview.status')}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-green-900">{t('privacy.overview.consent.granted')}</h4>
                <p className="text-sm text-green-700">
                  {new Date(state.lastUpdated).toLocaleDateString(language === 'pl' ? 'pl-PL' : 'en-US')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-medium text-blue-900">{t('privacy.overview.cookies.active')}</h4>
      ...
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('privacy.overview.preferences')}</h3>
        <div className="space-y-3">
          {(['necessary', 'functional', 'analytics', 'marketing'] as const).map((categoryId) => {
            ...
            const categoryDescs: Record<string, string> = {
              necessary: t('privacy.category.necessary.desc') || 'Wymagane do działania strony.',
              functional: t('privacy.category.functional.desc') || 'Personalizacja i wygoda.',
              analytics: t('privacy.category.analytics.desc') || 'Anonimowe statystyki.',
              marketing: t('privacy.category.marketing.desc') || 'Reklamy dopasowane do Ciebie.',
            };
            return (
              <div key={categoryId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <h4 className="font-medium text-gray-900">{categoryNames[categoryId]}</h4>
                  <p className="text-sm text-gray-600">{categoryDescs[categoryId]}</p>
                </div>
