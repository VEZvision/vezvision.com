...
export function DataTab({
  ...
  return (
    <div id="panel-data" role="tabpanel" className="p-6 space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('privacy.data.management')}</h3>

        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              <Download className="w-6 h-6 text-blue-600 mt-1" aria-hidden="true" />
              <div className="flex-1">
                <h4 className="font-medium text-blue-900 mb-1">{t('privacy.data.export.title')}</h4>
                ...
                >
                  ...
                  {isExporting ? t('privacy.data.export.loading') : t('privacy.data.export.button')}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              <Trash2 className="w-6 h-6 text-red-600 mt-1" aria-hidden="true" />
              <div className="flex-1">
                <h4 className="font-medium text-red-900 mb-1">{t('privacy.data.delete.title')}</h4>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-3">{t('privacy.data.consent.info')}</h4>
            <div className="space-y-2 text-sm text-gray-600">
              ...
              <div className="flex justify-between">
                ...
                <span>{new Date(state.lastUpdated).toLocaleString(language === 'pl' ? 'pl-PL' : 'en-US')}</span>
              </div>
            </div>
          </div>
        </div>
