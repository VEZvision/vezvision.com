...
export function PrivacyCenter({ className = '' }: PrivacyCenterProps) {
  ...
  const getActiveCookies = () => {
    return COOKIE_DEFINITIONS.filter(cookie => {
      ...
      return state.preferences[cookie.category as keyof typeof state.preferences];
    });
  };

  const tabs = [
    { id: 'overview' as const, label: t('privacy.center.overview'), icon: Eye },
    { id: 'cookies' as const, label: t('privacy.center.cookies'), icon: Database },
  ...
  return (
    <div className={`fixed inset-0 z-50 ${className}`}>
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <div
          className={`relative w-full max-w-6xl max-h-[90vh] bg-white rounded-xl shadow-2xl transition-all duration-300 ease-out ${isAnimating
        ...
        >
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div>
              <h2 id="privacy-center-title" className="text-2xl font-bold text-gray-900">
                {t('privacy.center.title')}
              </h2>
              <p id="privacy-center-description" className="mt-1 text-sm text-gray-600">
            ...
            <button
              type="button"
              onClick={actions.hidePrivacyCenterModal}
              className="p-2 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-lg transition-colors duration-200"
                ...
                {tabs.map((tab, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 text-left rounded-lg transition-colors duration-200 ${isActive
              ...
              <div className="mt-6 pt-4 border-t border-gray-200">
                ...
                <div className="space-y-1 text-xs text-gray-600">
                  <div className="flex items-center justify-between">
                    ...
                    <kbd className="px-1.5 py-0.5 font-mono bg-gray-200 border border-gray-300 rounded">Esc</kbd>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>{t('privacy.center.shortcuts.tabs')}</span>
                    <kbd className="px-1.5 py-0.5 font-mono bg-gray-200 border border-gray-300 rounded">Ctrl+1-4</kbd>
                  </div>
                </div>
              </div>
            </div>
