...
function ToggleSwitch({ id, checked, onChange, disabled = false, 'aria-describedby': ariaDescribedBy }: ToggleSwitchProps) {
  return (
    <button
      id={id}
      role="switch"
    ...
    >
      ...
      />
      {disabled && (
        <Check className="absolute inset-0 m-auto w-3 h-3 text-gray-500" aria-hidden="true" />
      )}
    </button>
  );
}
...
export function CookiePreferences({ className = '' }: CookiePreferencesProps) {
  ...
  const getCategoryColor = (categoryId: string) => {
    switch (categoryId) {
      case 'necessary': return 'text-black bg-gray-100';
      case 'functional': return 'text-black bg-gray-100';
      case 'analytics': return 'text-black bg-gray-100';
      case 'marketing': return 'text-black bg-gray-100';
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4 z-[10002]">
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
            <div>
              <h2 id="preferences-title" className="text-2xl font-bold text-black">
                {t('cookie.preferences.title')}
              </h2>
              <p id="preferences-description" className="mt-1 text-sm text-gray-600">
          ...
          >
             ...
             <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
               <div className="flex items-start gap-3">
                 <Info className="w-5 h-5 text-black mt-0.5 flex-shrink-0" aria-hidden="true" />
                 <div className="text-sm text-gray-800">
                   <p className="font-medium mb-1 text-black">{t('cookie.preferences.info.title')}</p>
                   <p>
                     {t('cookie.preferences.info.description')}
                   </p>
                 </div>
               </div>
             </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600" id={`desc-${category.id}`}>
                              {category.description}
                            </p>
                             {category.legalBasis && (
                              <p className="text-xs text-gray-500 mt-1">
                                {t('cookie.preferences.legalBasis')}: {category.legalBasis}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <ToggleSwitch
                            ...
                            checked={localPreferences[category.id as keyof CookiePreferencesType]}
                            onChange={(checked) => handlePreferenceChange(category.id as keyof CookiePreferencesType, checked)}
                    ...
                    {isExpanded && (
                      <div className="p-4 border-t border-gray-200 bg-white">
                        <div className="space-y-3 text-sm text-gray-600">
                          <p>{isNecessary ? t('cookie.preferences.alwaysActive') : t('cookie.preferences.optional')}.</p>
