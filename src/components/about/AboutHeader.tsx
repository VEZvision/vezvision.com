...
function AboutHeader() { const titleRef = useRef<HTMLDivElement>(null);
...
useEffect(() => {
  ...
  if (titleEl) {
    cleanups.push(registerRevealElement(titleEl, { once: true, amount: 0.3, onReveal: onTitleReveal }));
  }
  if (textEl) {
    cleanups.push(registerRevealElement(textEl, { once: true, amount: 0.3, onReveal: onTextReveal }));
  }
...
useEffect(() => {
  ...
  return (
    <section className={`py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 ${styles.section}`}>
      <div className="max-w-4xl mx-auto text-center">
        ...
        >
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl leading-relaxed sm:leading-relaxed md:leading-loose text-gray-800 font-medium text-center sm:text-justify px-2 sm:px-4 min-h-[120px] sm:min-h-[140px] md:min-h-[160px] lg:min-h-[180px]">
            ...
            <span className={`inline-block w-0.5 h-5 sm:h-6 md:h-7 lg:h-8 bg-gray-800 ml-1 ${showCursor ? 'opacity-100' : 'opacity-0'} transition-opacity duration-100`} />
          </p>
        </div>
      </div>
    </section>
