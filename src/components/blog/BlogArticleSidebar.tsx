...
type BlogArticleSidebarProps = {
  ...
  getPostTranslation: (post: BlogPostWithDetails, lang: 'pl' | 'en') => ReturnType<
    ReturnType<typeof import('@/hooks/useBlog').useBlog>['getPostTranslation']
  >;
};

export default function BlogArticleSidebar({ recentPosts, getPostTranslation }: BlogArticleSidebarProps) {
    ...
    <aside className="relative lg:block">
      <div className="sticky top-32 space-y-8">
                ...
                >
                  <div className="relative h-14 w-20 shrink-0 overflow-hidden rounded-lg shadow-sm">
                    <img
                      ...
                      onError={(event) => { (event.currentTarget as HTMLImageElement).src = logo; }}
                    />
                  </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <p className="mb-5 text-sm leading-6 text-gray-500">{t('newsletter.description')}</p>
          <Link
