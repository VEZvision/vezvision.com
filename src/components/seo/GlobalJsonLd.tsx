...
export function GlobalJsonLd() {
  ...
  const siteName = seo?.ogSiteName || identity?.siteName || seo?.siteTitle || 'VezVision'
  const logoUrl = safeImageUrl(identity?.logoUrl) || safeImageUrl(identity?.defaultOgImageUrl)
  ...
  if (contact?.email || contact?.phone) {
    organization.contactPoint = {
      ...
      availableLanguage: SUPPORTED_LOCALES.map((locale) => (locale === 'pl' ? 'Polish' : 'English')),
    }
  }
  ...
  return (
    <Helmet>
      <script type="application/ld+json">{safeJsonLd({ '@context': 'https://schema.org', '@graph': graph })}</script>
    </Helmet>
  )
}
