...
const PageSeo = ({ pageKey }: PageSeoProps) => {
  ...
  const canonicalUrl = safeAbsoluteHttpUrl(entry.canonical_url) || (siteUrl ? `${siteUrl}${location.pathname === '/' ? '' : location.pathname}` : '')
  ...
  const ogImage = safeImageUrl(entry.og_image_url) || safeImageUrl(identity?.defaultOgImageUrl) || safeImageUrl(identity?.logoUrl) || ''
