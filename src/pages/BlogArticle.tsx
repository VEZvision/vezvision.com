...
const BlogArticleSidebar = lazy(() => import('@/components/blog/BlogArticleSidebar'));

function BlogArticle() { const { slug } = useParams<{ slug: string }>();
const {
  post,
  isLoading,
  isError,
...
const { posts: recentPosts, getPostTranslation } = useBlogRecentPosts(4, post?.id);
...
if (isLoading) {
  return (
    <div className="min-h-[60vh] pt-32 flex justify-center items-center" aria-busy="true">
      <div
...
if (isError || isNotFound || !post) {
  const errorDescription = isError ? t('blog.article.load_error_desc') : t('blog.article.not_found_desc');
  ...
  return (
    <div className="min-h-[60vh] pt-32 flex items-center justify-center px-4">
      <DynamicPageSeo
        ...
        canonicalUrl={siteUrl ? `${siteUrl}${toLocalizedPath(`blog/${slug ?? ''}`)}` : toLocalizedPath(`blog/${slug ?? ''}`)}
      ...
      <div className="text-center w-full max-w-md">
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-12">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            {isError ? t('blog.article.load_error') : t('blog.article.not_found')}
          </h2>
  ...
  return (
    <section className="relative z-10 pt-32 pb-20 px-4 sm:px-6 lg:px-8" aria-labelledby="blog-article-title">
      ...
      structuredData={{
        ...
        author: { '@type': 'Organization', name: siteName, '@id': siteUrl ? `${siteUrl}/#organization` : undefined },
        publisher: { '@type': 'Organization', name: siteName, '@id': siteUrl ? `${siteUrl}/#organization` : undefined },
      }}
    />
    ...
    <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6 lg:gap-12">
      <article>
        ...
        <div className="prose prose-lg max-w-none prose-headings:text-gray-900 prose-p:text-gray-600 prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-img:shadow-md">
          <div dangerouslySetInnerHTML={{ __html: sanitizeCmsHtml(content) }} />
