...
function normalizeConfigRecord(
  ...
  return Object.fromEntries(
    Object.entries(value).filter(([, entryValue]) =>
      ["string", "boolean", "number"].includes(typeof entryValue),
    ),
...
export function normalizePageSections(entries: unknown[]): PageSectionsMap {
  const sections = entries.flatMap((entry) => {
    ...
    const sectionKey =
      typeof entry.section_key === "string" ? entry.section_key : "";
    ...
    return [
      {
        ...
        order_index:
          typeof entry.order_index === "number" ? entry.order_index : 0,
...
export function flattenCmsTranslations(pageSections: PageSectionsMap) {
  return Object.values(pageSections)
    .flat()
    .reduce(
      (acc, section) => {
...
export async function getPublicPageSections(): Promise<{
  data: unknown[];
  error: string | null;
}> {
