...
function registerSanitizeHooks(): void {
  ...
  DOMPurify.addHook("afterSanitizeAttributes", (node) => {
    if (node.tagName === "A") {
      ...
      if (target === "_blank") {
        ...
        if (!/\bnoopener\b/.test(rel) || !/\bnoreferrer\b/.test(rel)) {
          node.setAttribute(
            "rel",
            rel ? `${rel} noopener noreferrer`.trim() : "noopener noreferrer",
          );
        }
      }
    }
    ...
    if (node.tagName === "IMG") {
      ...
      node.setAttribute(
        "loading",
        node.getAttribute("loading") === "eager" ? "eager" : "lazy",
      );
      node.setAttribute("decoding", "async");
      if (!node.getAttribute("width")) node.setAttribute("width", "1200");
      if (!node.getAttribute("height")) node.setAttribute("height", "675");
    }
  });
