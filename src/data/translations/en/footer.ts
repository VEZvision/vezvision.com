export const footerTranslations = {
  "footer.subtitle":
    "Websites, apps and processes that make work clearer and support company growth.",
  "footer.tagline": "Technology, design and delivery in one team.",
  "footer.cta": "Start cooperation",
  "footer.legal.privacy": "Privacy Policy",
  "footer.legal.terms": "Terms of Service",
  "footer.legal.cookies": "Cookies",
  "newsletter.title": "Notes about technology",
  "newsletter.description":
    "Leave your email if you want practical notes about websites, apps, AI and working with technology. No spam.",
  "newsletter.placeholder": "Your email address",
  "newsletter.submit": "Subscribe",
  "newsletter.submitting": "Subscribing...",
  "newsletter.disclaimer":
    "By subscribing, you accept our Privacy Policy. You can unsubscribe at any time.",
  "newsletter.success": "You are on the list.",
  "newsletter.duplicate": "This email address is already subscribed.",
  "newsletter.error.empty": "Please enter an email address",
  "newsletter.error.invalid": "Please enter a valid email address",
  "newsletter.error.general": "We could not save this email. Try again later.",
  "newsletter.error.unexpected": "Something went wrong. Try again later.",
  "newsletter.page.title": "VezVision Newsletter",
  "newsletter.page.eyebrow": "Useful notes, no spam",
  "newsletter.page.description":
    "Leave your email if you want practical notes about websites, apps, AI and working with technology.",
  "newsletter.page.back": "Back to blog",
  "newsletter.page.highlight.new.title": "New articles",
  "newsletter.page.highlight.new.description":
    "We will write when there is a new blog post.",
  "newsletter.page.highlight.tips.title": "Practical notes",
  "newsletter.page.highlight.tips.description":
    "Short, concrete and free from marketing noise.",
  "newsletter.page.highlight.control.title": "Full control",
  "newsletter.page.highlight.control.description":
    "Unsubscribe anytime from the link in each message.",
  "newsletter.error.captcha": "Please confirm you are not a bot.",
  "contact.form.error.captcha": "Please confirm you are not a bot.",
  "blog.hero.badge": "BLOG & NEWS",
  "blog.hero.title": "Knowledge that drives innovation",
  "blog.hero.description":
    "Discover the latest trends in IT, AI, and marketing. Practical guides, case studies, and expert analyses.",
  "blog.hero.cta.aria": "Go to contact",
  "blog.hero.cta.text": "Contact us",
  "blog.articles.badge": "BLOG",
  "blog.articles.title.line1": "Latest",
  "blog.articles.title.line2.italic": "articles",
  "blog.articles.subtitle":
    "Discover our latest posts, analyses, and guides from the world of technology and business.",
  "blog.article.not_found": "Article not found",
  "blog.article.load_error": "An error occurred while loading the article",
  "blog.article.load_error_desc":
    "We could not load this article. Check your connection and try again.",
  "blog.article.not_found_desc": "Check the URL or go back to the blog.",
  "blog.article.back": "Back to blog",
  "blog.article.reading_time": "read",
  "blog.article.cta_title": "Want to implement similar solutions?",
  "blog.article.cta_desc": "Write to us. We can talk about your project.",
  "blog.article.cta_button": "Let's talk",
  "blog.article.recent": "Recent Articles",
  "blog.list.loading": "Loading articles...",
  "blog.list.error": "Error while loading articles",
  "blog.list.empty": "Articles will appear soon",
  "blog.list.empty_desc":
    "We are preparing practical content about websites, applications, automations and AI for business.",
  "blog.list.show_more": "Show more",
  "unsubscribe.title": "Unsubscribe from Newsletter",
  "unsubscribe.processing": "One moment...",
  "unsubscribe.confirm_question":
    "Do you want to remove this email address from the newsletter?",
  "unsubscribe.confirm_yes": "Yes, unsubscribe me",
  "unsubscribe.confirm_no": "Keep subscription",
  "unsubscribe.back_home": "Return to Home",
  "unsubscribe.retry": "Try again",
  "unsubscribe.no_token":
    "To unsubscribe, use the link from the newsletter email.",
  "unsubscribe.error_generic": "An error occurred.",
  "unsubscribe.success": "has been removed from the list.",
  "unsubscribe.error_result": "An error occurred while unsubscribing.",
  "legal.back": "Back",
  "legal.last_updated": "Last updated",
  "routeError.title": "Something went wrong",
  "routeError.message": "Sorry, we ran into an unexpected problem.",
  "routeError.unknown": "An unknown error occurred.",
  "routeError.home": "Home",
  "routeError.retry": "Try again",
  "notfound.logoAlt": "VezVision Logo",
  "notfound.title": "Page not found",
  "notfound.description":
    "Sorry, the page you are looking for does not exist or has been moved.",
  "notfound.cta.home": "Go back to the homepage",
  "notfound.cta.contact": "Contact us",
  "notfound.help.text": "You can also:",
  "notfound.help.services": "See our services",
  "notfound.help.portfolio": "See portfolio",
  "notfound.help.blog": "Read the blog",
} as const;
