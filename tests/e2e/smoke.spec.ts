...
test("unknown routes render a not-found page", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: /not found|nie znaleziono|nie została znaleziona/i,
    }),
  ).toBeVisible();
...
test("legacy routes without locale redirect to localized paths", async ({
  page,
}) => {
...
test("not-found page includes site footer navigation", async ({ page }) => {
  ...
  await expect(
    page
      ...
      .getByRole("link", { name: "VEZ", exact: true }),
  ).toHaveAttribute("href", "https://vezhq.com");
});
...
test("contact form shows validation when submitted empty", async ({ page }) => {
  await page.goto("/pl/contact");
  await page
    .getByRole("button", { name: /wyślij wiadomość|send message/i })
    .click();
  await expect(
    page.locator('input[name="fullName"][aria-invalid="true"]'),
  ).toBeVisible();
});
