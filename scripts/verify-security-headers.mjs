...
// Read the nonce generated by sync-security-headers (shared via .csp-nonce file).
const noncePath = path.join(root, '.csp-nonce')
const nonce = fs.existsSync(noncePath) ? fs.readFileSync(noncePath, 'utf8').trim() : null

const expectedCsp = buildContentSecurityPolicy(nonce)
